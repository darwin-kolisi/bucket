...
export default function EditTaskPage() {
  ...
  const targetProjectRoute = projectId ? `/projects/${projectId}` : '/projects';
  ...
  useEffect(() => {
    const loadProject = async () => {
      ...
      try {
        ...
        setTaskName(loadedTask.title || '');
        setDescription(loadedTask.description || '');
        setDueDate(toDateInput(loadedTask.dueDate));
        setPriority(loadedTask.priority || 'Medium');
        setSubtasks(Array.isArray(loadedTask.subtasks) ? loadedTask.subtasks : []);
        setHasInitialized(true);
      } catch (err) {
        setError('Task not found.');
        setIsTaskMissing(true);
    ...
    loadProject();
  }, [apiBase, projectId, taskId, task, setProjects]);
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
            <form
              ...
              <div className="task-form-stack">
                <section className="task-form-panel sm:shadow-sm">
            ...
            <aside className="space-y-4 hidden sm:block">
              <div className="surface-card rounded-2xl border border-gray-200 bg-white shadow-sm p-5">
                ...
                <div className="mt-4 rounded-xl border border-gray-200 surface-muted p-3 text-xs text-gray-500">
                  Update details to keep the board accurate.
                </div>
              </div>
            </aside>
          </div>
